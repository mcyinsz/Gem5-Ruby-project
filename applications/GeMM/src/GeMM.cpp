 ...
 */
void gemm_worker(double **A, double **B, double **C,
                 int tid, int threads,
                 int M, int N, int K)
{
    // Each thread computes rows from tid to M-1 with step 'threads'
...
void gemm_worker_1d(double *A, double *B, double *C,
                    int tid, int threads,
                    int M, int N, int K)
{
    // Each thread computes rows from tid to M-1 with step 'threads'
...
int main(int argc, char *argv[])
{
    ...
    for (int i = 0; i < cpus - 1; i++) {
        threads[i] = new thread(gemm_worker_1d, A, B, C, i, cpus, M, N, K);
    }

    // Execute the last thread with this thread context to appease SE mode
    gemm_worker_1d(A, B, C, cpus - 1, cpus, M, N, K);
